...
void execute_inst_ADD(size_t worker_number, operand_t op1, operand_t op2,
  ...
  char *op_str = "ADD";
  LF_PRINT_DEBUG("*** Worker %zu executing instruction: [Line %zu] %s %" PRIu64
                 " %" PRIu64 " %" PRIu64,
                 worker_number, *pc, op_str, op1.imm, op2.imm, op3.imm);
  reg_t *dst = op1.reg;
  reg_t *src = op2.reg;
  reg_t *src2 = op3.reg;
  *dst = *src + *src2;
...
void execute_inst_BGE(size_t worker_number, operand_t op1, operand_t op2,
  ...
  reg_t *_op1 = op1.reg;
  reg_t *_op2 = op2.reg;
  LF_PRINT_DEBUG("Worker %zu: BGE : operand 1 = %lld, operand 2 = %lld",
                 worker_number, *_op1, *_op2);
  if (_op1 != NULL && _op2 != NULL && *_op1 >= *_op2)
    *pc = op3.imm;
  else
    *pc += 1;
...
void execute_inst_DU(size_t worker_number, operand_t op1, operand_t op2,
  ...
  LF_PRINT_DEBUG("DU wakeup time: %lld, base: %lld, offset: %lld", wakeup_time,
                 *src, op2.imm);
  ...
  LF_PRINT_DEBUG("*** [Line %zu] Worker %zu delaying, current_physical_time: "
                 "%lld, wakeup_time: %lld, wait_interval: %lld",
                 *pc, worker_number, current_time, wakeup_time, wait_interval);
  if (wait_interval > 0) {
...
void execute_inst_EXE(size_t worker_number, operand_t op1, operand_t op2,
  ...
  LF_PRINT_DEBUG("*** [Line %zu] Worker %zu executing reaction", *pc,
                 worker_number);
  // function(args);
  LF_PRINT_DEBUG("*** [Line %zu] Worker %zu done executing reaction", *pc,
                 worker_number);
  *pc += 1; // Increment pc.
}
...
tag_t lf_tag_add(tag_t a, tag_t b) {
  ...
  if (b.time > 0) {
    // NOTE: The reason for handling this case is to "reset" the microstep
    // counter at each after delay.
...
tag_t lf_delay_strict(tag_t tag, interval_t interval) {
  ...
  if (interval != 0 && interval != NEVER && interval != FOREVER &&
