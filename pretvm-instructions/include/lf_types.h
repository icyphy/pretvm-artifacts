 ...
 * 2-clause</a>
 * @brief Type definitions that are widely used across different parts of the
 * runtime.
 *
 ...
 * See <a
 * href="https://github.com/lf-lang/reactor-c/wiki/Structs-in-the-Reactor-C-Runtime">
 ...
 *
 * If the value is 'present', it is an indicator that the port is present at the
 * given logical time. If the value is 'absent', it is an indicator that the
 * port is absent at the given logical time. If the value is 'unknown', it is
 * unknown whether the port is present or absent (e.g., in a distributed
 * application).
 *
 ...
 * If a reaction is 'queued', it is going to be executed at the current logical
 * time, but it has not started running yet. If a reaction is 'running', its
 * body is being executed.
 *
 * @note inactive must equal zero because it should be possible to allocate a
 * reaction with default values using calloc.
...
struct reaction_t {
  ...
  int number; // The number of the reaction in the reactor (0 is the first
              // reaction).
  // index_t index;                // Inverse priority determined by dependency
  // analysis. INSTANCE.
  size_t pos; // Current position in the priority queue. RUNTIME.
  reaction_t
      *last_enabling_reaction; // The last enabling reaction, or NULL if there
                               // is none. Used for optimization. INSTANCE.
  size_t num_outputs; // Number of outputs that may possibly be produced by this
                      // function. COMMON.
  bool **output_produced; // Array of pointers to booleans indicating whether
                          // outputs were produced. COMMON.
  int *triggered_sizes;  // Pointer to array of ints with number of triggers per
                         // output. INSTANCE.
  trigger_t ***triggers; // Array of pointers to arrays of pointers to triggers
                         // triggered by each output. INSTANCE.
  reaction_status_t status; // Indicator of whether the reaction is inactive,
                            // queued, or running. RUNTIME.
  interval_t deadline;  // Deadline relative to the time stamp for invocation of
                        // the reaction. INSTANCE.
  bool is_STP_violated; // Indicator of STP violation in one of the input
                        // triggers to this reaction. default = false. Value of
                        // True indicates to the runtime that this reaction
                        // contains trigger(s) that are triggered at a later
                        // logical time that was originally anticipated.
                        // Currently, this is only possible if logical
                        // connections are used in a decentralized federated
                        // execution. COMMON.
  reaction_function_t
      deadline_violation_handler; // Deadline violation handler. COMMON.
  reaction_function_t
      STP_handler; // STP handler. Invoked when a trigger to this reaction
                   // was triggered at a later logical time than originally
                   // intended. Currently, this is only possible if logical
                   // connections are used in a decentralized federated
                   // execution. COMMON.
  bool is_an_input_reaction; // Indicates whether this reaction is a network
                             // input reaction of a federate. Default is false.
  size_t worker_affinity; // The worker number of the thread that scheduled this
                          // reaction. Used as a suggestion to the scheduler.
 ...
 */
struct trigger_t {
  // token_template_t tmplt;  // Type and token information (template is a C++
  // keyword).
  reaction_t *
      *reactions; // Array of pointers to reactions sensitive to this trigger.
  int number_of_reactions; // Number of reactions sensitive to this trigger.
  bool is_timer; // True if this is a timer (a special kind of action), false
                 // otherwise.
  interval_t offset; // Minimum delay of an action. For a timer, this is also
                     // the maximum delay.
  interval_t period; // Minimum interarrival time of an action. For a timer,
                     // this is also the maximal interarrival time.
  ...
  lf_spacing_policy_t policy; // Indicates which policy to use when an event is
                              // scheduled too early.
  port_status_t status; // Determines the status of the port at the current
                        // logical time. Therefore, this value needs to be reset
                        // at the beginning of each logical time.
                        //
  // This status is especially needed for the distributed execution because the
  // receiver logic will need to know what it should do if it receives a message
  // with 'intended tag = current tag' from another federate.
  // - If status is 'unknown', it means that the federate has still no idea what
  // the status of
  //   this port is and thus has refrained from executing any reaction that has
  //   that port as its input. This means that the receiver logic can directly
  //   inject the triggered reactions into the reaction queue at the current
  //   logical time.
  // - If the status is absent, it means that the federate has assumed that the
  // port is 'absent'
  //   for the current logical time. Therefore, receiving a message with
  //   'intended tag = current tag' is an error that should be handled, for
  //   example, as a violation of the STP offset in the decentralized
  //   coordination.
  // - Finally, if status is 'present', then this is an error since multiple
  //   downstream messages have been produced for the same port for the same
  //   logical time.
#ifdef FEDERATED
  tag_t last_known_status_tag; // Last known status of the port, either via a
                               // timed message, a port absent, or a TAG from
                               // the RTI.
  tag_t
      intended_tag; // The amount of discrepency in logical time between the
                    // original intended trigger time of this trigger and the
                    // actual trigger time. This currently can only happen when
                    // logical connections are used using a decentralized
  ...
  instant_t
      physical_time_of_arrival; // The physical time at which the message has
                                // been received on the network according to the
                                // local clock. Note: The
                                // physical_time_of_arrival is only passed down
                                // one level of the hierarchy. Default: NEVER.
#endif
};
...
typedef struct self_base_t {
  ...
  struct reaction_t
      *executing_reaction; // The currently executing reaction of the reactor.
  environment_t *environment;
#if !defined(LF_SINGLE_THREADED)
  void *reactor_mutex; // If not null, this is expected to point to an
                       // lf_mutex_t. It is not declared as such to avoid a
                       // dependence on platform.h.
#endif
...
#if defined REACTOR_LOCAL_TIME   // FIXME: The output_ports pointers isnt
                                 // obviously related to local time
  tag_t tag;                     // The current tag of the reactor instance.
  lf_port_base_t **output_ports; // An array of pointers to output ports of this
                                 // reactor. Used to reset the is_present fields
